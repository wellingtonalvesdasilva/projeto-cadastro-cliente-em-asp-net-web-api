...
namespace Arquitetura.Business
{
        ...
        where TDTO : class
    {
        ...
        public void Editar(TDTO dto, bool salvarAlteracoes = true)
        {
            ValidarCamposObrigatorios(dto, ECrudOperacao.Editar);

            ValidarRegrasDeNegocio(dto, ECrudOperacao.Editar);

            if (typeof(TDTO) == typeof(TEntity))
                TransactionHelper.PersistirEmTransacao(this.Contexto, () => EditarModel(dto as TEntity, salvarAlteracoes));
            else
                TransactionHelper.PersistirEmTransacao(this.Contexto, () => EditarDTO(dto, salvarAlteracoes));
        }

        protected virtual void EditarModel(TEntity model, bool salvarAlteracoes = true)
        {
            repository.Update(model, salvarAlteracoes);
        }

        protected virtual TEntity EditarDTO(TDTO dto, bool salvarAlteracoes = true)
        {
            throw new NotImplementedException();
        }

        public void Excluir(TDTO dto, bool salvarAlteracoes = true)
        {
            ValidarRegrasDeNegocio(dto, ECrudOperacao.Excluir);

            if (typeof(TDTO) == typeof(TEntity))
                TransactionHelper.PersistirEmTransacao(this.Contexto, () => ExcluirModel(dto as TEntity, salvarAlteracoes));
            else
                TransactionHelper.PersistirEmTransacao(this.Contexto, () => ExcluirDTO(dto, salvarAlteracoes));
        }

        public void ExcluirModel(TEntity entity, bool salvarAlteracoes = true)
        {
            repository.Delete(entity, salvarAlteracoes);
        }

        public void ExcluirDTO(TDTO dto, bool salvarAlteracoes = true)
        {
            throw new NotImplementedException();
        }
    }
